...
```{r, eval = FALSE}
getLearnerModel(tunedForestModel)
getTree(getLearnerModel(tunedForestModel))
```
...
retailDF %>%
  ...
  pivot_longer(cols = c('meanSales', 'medianSales'), names_to = 'Measure', values_to = 'Value')
...
ggplot(retailDF %>%
  ...
  summarise(meanSales = mean(Sales),
            medianSales = median(Sales)) %>%
  pivot_longer(cols = c('meanSales', 'medianSales'), names_to = 'Measure', values_to = 'Value'),
  aes(x = Product, y = Value), fill = as.factor(Product)) +
  geom_bar(stat = 'identity') +
  facet_wrap(~ Measure) +
  labs(title = 'Average & Median Sales of each Product')
```

```{r}
retailDF %>%
  group_by(Product, In_Store_Promo) %>%
  summarise(meanSales = mean(Sales),
            medianSales = median(Sales)) %>%
  pivot_longer(cols = c('meanSales', 'medianSales'), names_to = 'Measure', values_to = 'Value') %>%
  as.data.frame()
...
ggplot(retailDF %>%
  ...
  summarise(meanSales = mean(Sales),
            medianSales = median(Sales)) %>%
  pivot_longer(cols = c('meanSales', 'medianSales'), names_to = 'Measure', values_to = 'Value'),
  aes(x = Product, y = Value, fill = as.factor(In_Store_Promo))) +
  geom_bar(position = 'dodge', stat = 'identity') +
  facet_wrap(~ Measure) +
  labs(title = 'Average & Median Sales of each Product \n w/ Store Promotions', fill = 'Store Promotion') +
  scale_fill_manual(labels = c('No', 'Yes'), values = c('red', 'blue'))
```

```{r}
retailDF %>%
  group_by(PriceDiscount) %>%
  summarise(meanSales = mean(Sales),
            medianSales = median(Sales))
ggplot(retailDF %>%
  group_by(PriceDiscount) %>%
  summarise(meanSales = mean(Sales),
            medianSales = median(Sales)) %>%
  pivot_longer(cols = c('meanSales', 'medianSales'), names_to = 'Measure', values_to = 'Value'),
  aes(x = PriceDiscount, y = Value), fill = as.factor(PriceDiscount)) +
  geom_bar(stat = 'identity') +
  facet_wrap(~ Measure) +
  labs(title = 'Average & Median Sales based on Price Discounts')
```

```{r}
retailDF %>%
  group_by(Product, Catalogue_Promo) %>%
  summarise(meanSales = mean(Sales),
            medianSales = median(Sales)) %>%
  pivot_longer(cols = c('meanSales', 'medianSales'), names_to = 'Measure', values_to = 'Value') %>%
  as.data.frame()
...
ggplot(retailDF %>%
  ...
  summarise(meanSales = mean(Sales),
            medianSales = median(Sales)) %>%
  pivot_longer(cols = c('meanSales', 'medianSales'), names_to = 'Measure', values_to = 'Value'),
  aes(x = Product, y = Value, fill = as.factor(Catalogue_Promo))) +
  geom_bar(position = 'dodge', stat = 'identity') +
  facet_wrap(~ Measure) +
  labs(title = 'Average & Median Sales of each Product \n w/ Catalogue Promotions', fill = 'Catalogue Promotion') +
  scale_fill_manual(labels = c('No', 'Yes'), values = c('red', 'blue'))
```

```{r}
retailDF %>%
  group_by(Product, Store_End_Promo) %>%
  summarise(meanSales = mean(Sales),
            medianSales = median(Sales)) %>%
  pivot_longer(cols = c('meanSales', 'medianSales'), names_to = 'Measure', values_to = 'Value') %>%
  as.data.frame()
...
ggplot(retailDF %>%
  ...
  summarise(meanSales = mean(Sales),
            medianSales = median(Sales)) %>%
  pivot_longer(cols = c('meanSales', 'medianSales'), names_to = 'Measure', values_to = 'Value'),
  aes(x = Product, y = Value, fill = as.factor(Store_End_Promo))) +
  geom_bar(position = 'dodge', stat = 'identity') +
  facet_wrap(~ Measure) +
  labs(title = 'Average & Median Sales of each Product \n w/ Ending Promotions', fill = 'Ending Promotion') +
  scale_fill_manual(labels = c('No', 'Yes'), values = c('red', 'blue'))
```

```{r}
retailDF %>%
  group_by(Product, Covid_Flag) %>%
  summarise(meanSales = mean(Sales),
            medianSales = median(Sales)) %>%
  pivot_longer(cols = c('meanSales', 'medianSales'), names_to = 'Measure', values_to = 'Value') %>%
  as.data.frame()
...
ggplot(retailDF %>%
  ...
  summarise(meanSales = mean(Sales),
            medianSales = median(Sales)) %>%
  pivot_longer(cols = c('meanSales', 'medianSales'), names_to = 'Measure', values_to = 'Value'),
  aes(x = Product, y = Value, fill = as.factor(Covid_Flag))) +
  geom_bar(position = 'dodge', stat = 'identity') +
  facet_wrap(~ Measure) +
  labs(title = 'Average & Median Sales of each Product \n w/ Store-Wide Covid Risk', fill = 'Covid Risk') +
  scale_fill_manual(labels = c('No', 'Yes'), values = c('red', 'blue'))
```

```{r}
retailDF %>%
  group_by(Product, Christmas) %>%
  summarise(meanSales = mean(Sales),
            medianSales = median(Sales)) %>%
  pivot_longer(cols = c('meanSales', 'medianSales'), names_to = 'Measure', values_to = 'Value') %>%
  as.data.frame()
...
ggplot(retailDF %>%
  ...
  summarise(meanSales = mean(Sales),
            medianSales = median(Sales)) %>%
  pivot_longer(cols = c('meanSales', 'medianSales'), names_to = 'Measure', values_to = 'Value'),
  aes(x = Product, y = Value, fill = as.factor(Christmas))) +
  geom_bar(position = 'dodge', stat = 'identity') +
  facet_wrap(~ Measure) +
  labs(title = 'Average & Median Sales of each Product \n on Christmas Day', fill = 'Christmas Day') +
  scale_fill_manual(labels = c('No', 'Yes'), values = c('red', 'blue'))
```
...
ggplot(retailDF %>%
  ...
  summarise(meanSales = mean(Sales),
            medianSales = median(Sales)) %>%
  pivot_longer(cols = c('meanSales', 'medianSales'), names_to = 'Measure', values_to = 'Value'),
  aes(x = Week, y = Value), fill = as.factor(Week)) +
  geom_bar(stat = 'identity') +
  facet_wrap(~ Measure) +
  labs(title = 'Average & Median Sales by Week')
```
...
ggplot(retailDF %>%
  ...
  summarise(meanSales = mean(Sales),
            medianSales = median(Sales)) %>%
  pivot_longer(cols = c('meanSales', 'medianSales'), names_to = 'Measure', values_to = 'Value'),
  aes(x = Product, y = Value), fill = as.factor(Measure)) +
  geom_bar(position = 'dodge', stat = 'identity') +
  facet_wrap(~ Year) +
  labs(title = 'Average & Median Sales of each Product \n by Year', fill = 'Catalogue Promotion') +
  scale_fill_manual(labels = c('No', 'Yes'), values = c('red', 'blue'))
```
